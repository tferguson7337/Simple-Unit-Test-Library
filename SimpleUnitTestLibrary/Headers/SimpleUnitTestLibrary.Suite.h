...
namespace SimpleUnitTestLibrary
{
    namespace Concepts
    {
        ...
        concept ValidUnitTestRangeSource = std::ranges::range<RangeT> &&
            std::convertible_to<std::remove_cvref_t<std::ranges::range_value_t<RangeT>>, Test>;
    }
        ...
        struct RunResults
        {
            ...
            [[nodiscard]] constexpr explicit operator bool() const noexcept
            {
                return std::ranges::all_of(
                    m_UnitTests,
                    [](_In_ const Test& unitTest) static constexpr
            ...
            [[nodiscard]] std::string ToString(
                _In_ const std::size_t spaces = 0) const
            {
                std::string ret;
                ...
                for (const Test& test : m_UnitTests)
                {
                    ...
                    ret += std::format("{:<{}}{:<{}}{}\n", "", spaces + 2, test.GetTestName(), std::min(std::size_t{72}, maxTestNameLength + 1), result.ToString(spaces + 2));
                }

                auto GetTotalFailureCount = [&resultTypeCounts]() constexpr -> std::uint64_t
                {
                    const std::uint64_t setupFailures = resultTypeCounts[static_cast<std::size_t>(ResultType::SetupFailure)];
                    const std::uint64_t testFailures = resultTypeCounts[static_cast<std::size_t>(ResultType::TestFailure)];
                    const std::uint64_t cleanupFailures = resultTypeCounts[static_cast<std::size_t>(ResultType::CleanupFailure)];
                    const std::uint64_t unhandledExceptions = resultTypeCounts[static_cast<std::size_t>(ResultType::UnhandledException)];
                    return setupFailures + testFailures + cleanupFailures + unhandledExceptions;
                };
...
template <>
struct std::formatter<SimpleUnitTestLibrary::Suite::RunResults>
{
    constexpr auto parse(
        _In_ const std::format_parse_context& ctx)
    {
        return ctx.begin();
    }
